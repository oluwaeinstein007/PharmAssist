...
export const LogPurchaseTool = {
    name: "LOG_PURCHASE",
    description:
        "Log a medicine purchase for analytics and inventory management and planning.",
    parameters: LogPurchaseParamsSchema,
    execute: async (args: z.infer<typeof LogPurchaseParamsSchema>) => {

        console.log(`[LOG_PURCHASE] Called with: ${JSON.stringify(args)}`);
        const logPurchaseService = new LogPurchaseService();

        try {
            const medsList = await logPurchaseService.addLog({
